...
async function getValthera(options: { "config-path": string; config: string; dir: string }) {
    let cfg: Cfg = {
        dbs: {}
    };
    ...
    const currentConfig: Opts =
        ...
        {
            def: "dir",
            opts: [options.dir || ".", { format: "json5:x" }]
        };

    if (options.config) {
        const data = options.config.split(",").map(data => data.split("=")).reduce((acc, [key, value]) => {
            acc[key] = value;
            return acc;
...
program
    ...
    .option("-p, --config-path <path>", "Path to the config file", "./valthera.cli.yml");
